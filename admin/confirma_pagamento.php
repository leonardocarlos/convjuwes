...
if($_POST['tipo_pagamento'] == ''){
    ...
    $inscricaoQuery = consultaDados("select
        ...
        INNER JOIN evento AS e ON i.id_evento = '$id_evento'
        INNER JOIN inscritos AS u ON i.id_inscrito = u.id
        where 
        i.pagto_confirma = 0 and            
        e.ativo = 1
        order by
        u.nome
        ");
...
<div class="span4">
    <section>
        <strong>Total de Participantes:</strong> <?php //echo $total['total_quantidade']; ?> Inscritos <br><br>
...
<div class="span12">
 <table class="table table-striped table-condensed" width="">
     ...
     <tbody>
     <?php while($participante = mysql_fetch_array($inscricaoQuery)) {  
         ...
         ?>
     <tr>        
     <th><?php echo $participante['usuario'];?></th>
     ...
     <th>
        ...
        <a class="btn btn-small btn-success" href="?p=confirmar_inscricao&acao=editar&id=<?php echo $participante['id']; ?>" title="Clique para Editar"><i class="icon-edit icon-white"></i></a> 
